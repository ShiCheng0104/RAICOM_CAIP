...
\begin{enumerate}
  ...
  \item \textbf{敏感接口认证：}配置\texttt{FRAUDSIM\_API\_KEY}后，预测、反馈、重载、演示任务、模型发布和回滚等接口要求\texttt{X-API-Key}；
  \item \textbf{双通道审计：}敏感操作同时写入本地\texttt{audit.jsonl}和Kafka的\texttt{audit\_events}主题，Kafka不可用时仍保留本地审计；
