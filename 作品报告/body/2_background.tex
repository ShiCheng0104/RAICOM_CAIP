...
\begin{table}[H]
  ...
  \begin{tabularx}{0.98\textwidth}{>{\raggedright\arraybackslash}p{2.1cm}>{\raggedright\arraybackslash}p{3.1cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}p{3.1cm}}
    \toprule
    数据源 & 数据属性 & 在FP-FraudSim中的作用 & 主要产物 \\
    \midrule
...
\begin{table}[H]
  ...
  {\songti\zihao{5}
  \begin{tabularx}{0.94\textwidth}{>{\raggedright\arraybackslash}X>{\raggedleft\arraybackslash}p{2.5cm}>{\raggedright\arraybackslash}p{5.1cm}}
    \toprule
    数据产物或属性 & 数量 & 说明 \\
    \midrule
    统一交易主表 & 830,733笔 & 含810,133笔基础融合交易和20,600笔七类注入交易 \\
    欺诈/正常/无标签交易 & 67,066 / 747,667 / 16,000笔 & 有标签部分欺诈率为8.23\% \\
    用户/商户画像 & 639,656 / 65,179条 & 从交易实体与聚合统计构建 \\
    设备/IP画像 & 269,211 / 182,571条 & 支撑共享环境和异常终端识别 \\
    图节点/图边 & 2,765,530 / 1,966,057条 & 融合交易派生边、公开图源与注入证据边 \\
    训练/验证/测试/无标签集 & 566,795 / 124,289 / 123,649 / 16,000笔 & 有标签样本严格按事件时间切分 \\
    \bottomrule
  \end{tabularx}
  }
\end{table}

上述规模记录在增强数据集清单中，属于可在比赛环境中复现的MVP确定性采样版本，而非八个来源的全量简单汇总。构建脚本同时支持\texttt{full}模式及按来源配置容量上限，便于在有限资源下开展功能验证，或在更高配置环境中扩大交易量和图边规模。
