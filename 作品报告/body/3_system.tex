...
\begin{figure}[H]
  \centering
  \resizebox{\textwidth}{!}{%
  \begin{tikzpicture}[node distance=0.62cm and 0.65cm]
    \node[flowcard=flowcyan] (source) {\textcolor{white}{\bfseries 数据生成}\nodepart{second}正常交易\\欺诈场景注入};
    \node[flowcard=flowblue, right=of source] (kafka) {\textcolor{white}{\bfseries Kafka事件总线}\nodepart{second}交易主题\\结果与告警主题};
    \node[flowcard=flowpurple, right=of kafka] (flink) {\textcolor{white}{\bfseries Flink实时计算}\nodepart{second}事件时间与窗口\\特征拼接};
    \node[flowcard=floworange, right=of flink] (api) {\textcolor{white}{\bfseries 模型服务}\nodepart{second}风险评分\\理由码与阈值};
    \node[flowcard=flowgreen, right=of api] (dashboard) {\textcolor{white}{\bfseries 风险看板}\nodepart{second}告警研判\\人工反馈};
    \node[flowcard=flowblue, below=0.9cm of flink] (redis) {\textcolor{white}{\bfseries Redis画像库}\nodepart{second}用户、设备、IP\\商户与图特征};
    \node[flowcard=flowpurple, below=0.9cm of api] (training) {\textcolor{white}{\bfseries 模型训练管理}\nodepart{second}候选训练、评估\\发布与回滚};
    \draw[flowarrow] (source) -- (kafka); \draw[flowarrow] (kafka) -- (flink);
    \draw[flowarrow] (flink) -- (api); \draw[flowarrow] (api) -- (dashboard);
    \draw[flowarrow] (redis) -- (flink); \draw[flowarrow] (redis) -- (api);
    \draw[flowarrow] (dashboard.south) |- (training.east);
    \draw[flowarrow] (training) -- (api);
  \end{tikzpicture}
  }
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.96\textwidth]{pic/dashboard_reason_codes.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.76\textwidth]{pic/dashboard_fraud_groups.jpeg}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.90\textwidth]{pic/dashboard_overview.jpeg}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.74\textwidth]{pic/dashboard_replay_feedback.jpeg}
