...
\begin{table}[H]
  ...
  \begin{tabularx}{0.95\textwidth}{>{\raggedright\arraybackslash}p{4.6cm}>{\raggedright\arraybackslash}X}
    ...
    \texttt{fraudsim/simulator/generate.py} & 可调参数交易流、团伙结构和独立标签生成。 \\
    \texttt{fraudsim/threshold\_sandbox.py} & 两级业务阈值下的决策量、效果与工作量计算。 \\
    \texttt{fraudsim/graphsage\_experiment.py} & GraphSAGE受控采样、旁路训练和节点嵌入输出。 \\
    ...
    \texttt{deploy/k8s/} & 边缘实时侧与中心训练侧Kubernetes演示清单。 \\
    \texttt{tests/} & API、特征、窗口、微批、仿真、阈值、图学习和K8s结构测试。 \\
    \bottomrule
  \end{tabularx}
\end{table}
